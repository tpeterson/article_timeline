(function() {
  ...
  function compareEqual(new_graf_arr, old_graf_arr) {
    ...
    new_graf_arr.forEach(function(new_sentence) {
      ...
      var old_old_sentence = old_sentence.toString();

      var old_sentence_arr = old_old_sentence.split(/\s/);
      ...
      old_old_sentence.split(/\s/).forEach(function(old_sentence_word) {
        if (!edited_sentence_arr.contains(old_sentence_word)) {
          cut_sentence_arr.push("<s>" + old_sentence_word + "</s>");
          edited_sentence_arr.splice(old_old_sentence.split(/\s/).indexOf(old_sentence_word), 0, "<s>" + old_sentence_word + "</s>")
        }
      });
  ...
  function compareUnequal(new_graf_arr, old_graf_arr) {
    // NEW VERSION
    if (new_graf_arr.length > old_graf_arr.length) {
      return findUnmatchedSentence(new_graf_arr, old_graf_arr);
    } else if (new_graf_arr.length < old_graf_arr.length) {
      return findUnmatchedSentence(old_graf_arr, new_graf_arr);
    } else {
      return false;
    }

    function findUnmatchedSentence(long_graf_arr, short_graf_arr) {
      var all_sentences_info = [];
      var num_unmatched_sentences = long_graf_arr.length - short_graf_arr.length;

      long_graf_arr.forEach(function(long_sentence) {
        var long_sentence_arr = long_sentence.split(/\s/);
        ...
        short_graf_arr.forEach(function(short_sentence) {
          var short_sentence_arr = short_sentence.split(/\s/);

          var check_num_matches = long_sentence_arr.filter(function(long_sentence_word) {
            return short_sentence_arr.contains(long_sentence_word);
          });
          ...
          var new_match_percent = getMatchPercent(check_num_matches.length, short_sentence_arr.length);
          var prev_match_percent = getMatchPercent(sentence_info.num_matches, short_sentence_arr.length);

          if (new_match_percent > prev_match_percent) {
            sentence_info.arr_pos = long_graf_arr.indexOf(long_sentence);
      ...
      var new_long_graf_arr = (function() {
        var sliced_graf = [];
        var reference_graf = long_graf_arr;
        var i = 0;

        unmatched_sentences.forEach(function(sentence) {
          var saved_graf = long_graf_arr.slice(i, sentence.arr_pos);
      ...
      var final_graf_arr = [];//compareEqual(new_long_graf_arr, short_graf_arr);

      if (new_graf_arr.length > old_graf_arr.length) {
          final_graf_arr = compareEqual(new_long_graf_arr, short_graf_arr);
      } else if (new_graf_arr.length < old_graf_arr.length  && new_long_graf_arr.length === short_graf_arr.length) {
          final_graf_arr = compareEqual(short_graf_arr, new_long_graf_arr);
      }

      unmatched_sentences.forEach(function(sentence) {
        if (new_graf_arr.length > old_graf_arr.length) {
          //final_graf_arr = compareEqual(new_long_graf_arr, short_graf_arr);
          var processed_unmatched = "<strong>" + long_graf_arr[sentence.arr_pos] + "." + "</strong>";
          final_graf_arr.splice(sentence.arr_pos, 0, processed_unmatched);
        } else if (new_graf_arr.length < old_graf_arr.length && new_long_graf_arr.length === short_graf_arr.length) {
          //final_graf_arr = compareEqual(short_graf_arr, new_long_graf_arr);
          var processed_unmatched = "<s>" + long_graf_arr[sentence.arr_pos] + "." + "</s>";
          final_graf_arr.splice(sentence.arr_pos, 0, processed_unmatched);
        } else {
          return false;
        }
